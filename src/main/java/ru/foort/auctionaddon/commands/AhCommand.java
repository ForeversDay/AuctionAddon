    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (sub) {
            case "help" -> {
                ...
                for (String line : plugin.getConfig().getStringList("messages.help")) player.sendMessage(Color.translate(line.replace("%sec%", String.valueOf(sec))));
                return true;
            }
            case "author", "authors" -> {
                for (String line : plugin.getConfig().getStringList("messages.author")) player.sendMessage(Color.translate(line));
                return true;
            }
            case "sell", "dsell" -> {
                if (args.length < 2) { player.sendMessage(Color.translate(plugin.getConfig().getString(sub + "_usage"))); return true; }
                long priceL = utils.parseAmount(args[1]);
                if (priceL < plugin.getConfig().getInt("settings.sell_min-price", 10)) { player.sendMessage(Color.translate(plugin.getConfig().getString("messages.min_price").replace("%min%", String.valueOf(plugin.getConfig().getInt("settings.sell_min-price", 10))))); return true; }
                if (priceL > plugin.getConfig().getInt("settings.sell_max-price", 100000000)) { player.sendMessage(Color.translate(plugin.getConfig().getString("messages.max_price").replace("%max%", String.valueOf(plugin.getConfig().getInt("settings.sell_max-price", 100000000))))); return true; }
                int price = (int) priceL;
                ItemStack hand = player.getInventory().getItemInMainHand();
                if (hand == null || hand.getType().isAir()) { player.sendMessage(Color.translate(plugin.getConfig().getString("messages.no_item"))); return true; }
                if (Main.getBlackList().stream().anyMatch(tag -> hand.getType().name().equalsIgnoreCase(tag))) { player.sendMessage(Color.translate(plugin.getConfig().getString("messages.item_in_black_list"))); return true; }
                ...
                if (sub.equals("sell")) {
                    try {
                        ...
                        } else { player.getInventory().addItem(hand); }
                    } catch (Exception ex) { player.getInventory().addItem(hand); ex.printStackTrace(); }
                } else {
            ...
            case "search" -> {
                if (args.length < 2) { player.sendMessage(Color.translate(plugin.getConfig().getString("messages.search_usage"))); return true; }
                ...
                if (resultTags.isEmpty()) { player.sendMessage(Color.translate(plugin.getConfig().getString("messages.search_no_item"))); return true; }
                ...
                var menu = menuLoader.getMenu(homeMenuId);
                if (menu == null) { player.sendMessage(Color.translate("&fЭх, опять… То ли я что-то намудрил, то ли ты забыл в Main поменять название меню на своё.")); return true; }
            ...
            default -> {
                ...
                else uuid = UUID.nameUUIDFromBytes(("OfflinePlayer:" + sub).getBytes(StandardCharsets.UTF_8));
                User user = Main.getStorage().getUser(uuid);
                if (user == null) { player.sendMessage(Color.translate("&cИгрок не найден!")); return true; }
                var menu = menuLoader.getMenu(viewMenuId);
                if (menu == null) { player.sendMessage(Color.translate("&fЭх, опять… То ли я что-то намудрил, то ли ты забыл в Main поменять название меню на своё.")); return true; }
                var m = menu.create(player, null);
                if (m instanceof PlayerItemsView view) { view.setUuid(uuid); view.setName(user.getNickName()); }
                m.open();
                return true;
            }
        }
